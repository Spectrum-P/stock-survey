...
export function DuplicateCompletedFlatButton({ surveyId, sourceName }: { surveyId: string; sourceName: string }) {
  ...
  return <>
    ...
    {open ? <div className="fixed inset-0 z-50 grid place-items-center bg-slate-950/55 p-4"><form onSubmit={duplicate} className="w-full max-w-md text-left rounded-2xl border border-[var(--line)] bg-[var(--surface)] p-6 shadow-xl"><p className="eyebrow text-left">Copy completed flat</p><h2 className="mt-2 text-left text-xl font-semibold">Duplicate {sourceName}</h2><p className="mt-2 max-w-sm text-left text-sm text-[var(--ink-muted)]">The new flat receives the completed survey’s assessment data, ready for review. Photographs are not copied.</p><div className="mt-5 text-left"><Field label="New flat name" htmlFor={`duplicate-flat-${surveyId}`} required><Input id={`duplicate-flat-${surveyId}`} value={name} onChange={(event) => setName(event.target.value)} placeholder="Flat 20A" pattern="[A-Za-z0-9 ]+" required autoFocus /></Field></div>{error ? <p className="mt-4 rounded-lg bg-red-50 p-3 text-sm text-red-800" role="alert">{error}</p> : null}<div className="mt-6 flex justify-end gap-2"><Button type="button" variant="secondary" onClick={() => setOpen(false)}>Cancel</Button><Button type="submit" disabled={busy}>{busy ? "Duplicating" : "Duplicate flat"}</Button></div></form></div> : null}
  </>;
}
