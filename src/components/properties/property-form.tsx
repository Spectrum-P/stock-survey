...
export function PropertyForm() {
  ...
  const { register, handleSubmit, formState: { errors, isSubmitting } } = useForm<PropertyInput>({ resolver: zodResolver(propertySchema), defaultValues: { town: "", postcode: "" } });
  async function onSubmit(values: PropertyInput) {
    setServerError("");
    const response = await authenticatedFetch("/api/properties", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify(values) });
    if (!response.ok) { const body = await response.json().catch(() => null); setServerError(body?.error ?? "Could not create the property. Try again."); return; }
  ...
  return <form onSubmit={handleSubmit(onSubmit)} className="grid gap-6 xl:grid-cols-[1fr_340px]">
    <Card className="grid gap-5 p-5 sm:p-7">
      ...
      <div className="grid gap-5 sm:grid-cols-2">
        ...
        <Field label="Property type" htmlFor="propertyType" helper="Used by dashboard filters."><Input id="propertyType" {...register("propertyType")} placeholder="Supported housing" /></Field>
      ...
      <div className="flex flex-col-reverse gap-2 border-t border-[var(--line)] pt-5 sm:flex-row sm:justify-end"><Link href="/properties"><Button variant="secondary"><ArrowLeft size={18} />Cancel</Button></Link><Button type="submit" disabled={isSubmitting}><Check size={18} />{isSubmitting ? "Saving" : "Create property"}</Button></div>
    </Card>
    <aside><Card className="sticky top-24 p-5"><div className="grid size-11 place-items-center rounded-xl bg-blue-50 text-[var(--brand)] dark:bg-blue-950/60"><Buildings size={23} weight="duotone" /></div><h2 className="mt-5 text-lg font-semibold">What happens next</h2><ol className="mt-4 grid gap-4 text-sm leading-6 text-[var(--ink-muted)]"><li><strong className="text-[var(--ink)]">1. Add units</strong><br />Generate numbered flats or enter rooms individually.</li><li><strong className="text-[var(--ink)]">2. Start a survey</strong><br />Open any unit and create its inspection.</li><li><strong className="text-[var(--ink)]">3. Track progress</strong><br />Review completion and findings across the property.</li></ol></Card></aside>
  </form>;
}
