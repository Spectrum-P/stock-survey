...
export function RecordsTable({ records, exportOptions }: { records: SurveyRecord[]; exportOptions: ExcelExportPropertyOption[] }) {
  ...
  return <>
    <Card className="overflow-hidden p-0">
      ...
      <div className="grid gap-3 border-b border-[var(--border)] p-4 lg:grid-cols-[minmax(260px,1fr)_150px_150px_150px_170px_150px_150px_auto]">
        ...
        <Field label="Condition" hideLabel><select className="field" value={condition} onChange={(event) => setCondition(event.target.value)}><option value="">All conditions</option>{["A", "B", "C", "D"].map((value) => <option key={value}>{value}</option>)}</select></Field>
        <Field label="Priority" hideLabel><select className="field" value={priority} onChange={(event) => setPriority(event.target.value)}><option value="">All priorities</option>{["1", "2", "3", "4"].map((value) => <option key={value}>{value}</option>)}</select></Field>
        <Field label="Status" hideLabel><select className="field" value={status} onChange={(event) => setStatus(event.target.value)}><option value="">All statuses</option><option value="in_progress">In progress</option><option value="completed">Completed</option></select></Field>
        <Field label="Property" hideLabel><select className="field" value={property} onChange={(event) => setProperty(event.target.value)}><option value="">All properties</option>{[...new Set(records.map((record) => record.property))].sort().map((value) => <option key={value}>{value}</option>)}</select></Field>
        <Field label="Building" hideLabel><select className="field" value={building} onChange={(event) => setBuilding(event.target.value)}><option value="">All buildings</option>{[...new Set(records.map((record) => record.building))].sort().map((value) => <option key={value}>{value}</option>)}</select></Field>
        <Field label="Property type" hideLabel><select className="field" value={propertyType} onChange={(event) => setPropertyType(event.target.value)}><option value="">All property types</option>{[...new Set(records.map((record) => record.propertyType).filter((value) => value !== "Not recorded"))].sort().map((value) => <option key={value}>{value}</option>)}</select></Field>
