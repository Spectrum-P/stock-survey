...
import { calculateLifecycle } from "@/lib/lifecycle";
...
function newDraft(surveyId: string, elementName: string, sourceCatalog = fallbackCatalog): SurveyElementDraft {
  ...
  return {
    id: crypto.randomUUID(), surveyId, category: selected.category, element: selected.name, categoryId: undefined, elementId: undefined, accessibility: "inspection_required", accessibilityReason: "", constructionType: "", constructionNotes: "", installationYear: undefined, typicalLifeYears: selected.lifespan, lifeReference: selected.lifespanLabel, remainingLife: undefined, replacementYear: undefined, replacementYearProvided: false, planningHorizon: "", planningOverrideReason: "", estimatedCost: undefined, costBasis: "", defects: [{ id: crypto.randomUUID(), defectType: "No defect observed", cause: "", condition: "A", priority: "4", notes: "", photoIds: [] }], recommendedWorks: "", generalNotes: "", accessLimitations: "", furtherInvestigation: false, mediaIds: [], status: "not_started", version: 0, updatedAt: new Date().toISOString()
  };
}

function furthestAllowedStep(draft: SurveyElementDraft) {
  if (!draft.category || !draft.element) return 0;
  if (!draft.constructionType.trim() || (!draft.replacementYearProvided && (!draft.installationYear || !draft.typicalLifeYears))) return 1;
  if (draft.defects.some((finding) => !finding.condition || !finding.priority)) return 2;
  const calculated = calculateLifecycle(draft.installationYear, draft.typicalLifeYears, new Date().getFullYear(), draft.replacementYearProvided ? draft.replacementYear : undefined);
  if (!draft.planningHorizon || (draft.planningHorizon !== calculated.suggestedHorizon && !draft.planningOverrideReason.trim())) return 3;
  return 5;
}
...
export function ElementStepper({ surveyId, initialElement, initialDraft, catalogItems }: { surveyId: string; initialElement: string; initialDraft?: SurveyElementDraft; catalogItems?: CatalogElement[] }) {
  ...
  useEffect(() => {
    ...
    async function hydrate() {
      ...
      if (active) {
        const hydratedDraft = stored ?? initialDraft ?? newDraft(surveyId, initialElement, referenceCatalog);
        setDraft(hydratedDraft);
        const savedStep = Number(window.localStorage.getItem(`survey-step:${surveyId}:${initialElement}`) ?? 0);
        setStep(Math.min(furthestAllowedStep(hydratedDraft), 5, Math.max(0, savedStep)));
        setLoading(false);
      }
    }
    hydrate();
  ...
  const lifecycle = useMemo(() => calculateLifecycle(draft?.installationYear, draft?.typicalLifeYears, new Date().getFullYear(), draft?.replacementYearProvided ? draft.replacementYear : undefined), [draft?.installationYear, draft?.replacementYear, draft?.replacementYearProvided, draft?.typicalLifeYears]);
  ...
  function changeCategory(category: string) {
    ...
    patchDraft({ category, categoryId: undefined, customComponentName: undefined, element: first.name, elementId: undefined, customElementName: undefined, typicalLifeYears: first.lifespan, lifeReference: first.lifespanLabel, constructionType: "", defects: [] });
  }
  function changeElement(element: string) {
    ...
    patchDraft({ element, category: item.category, categoryId: undefined, customComponentName: undefined, elementId: undefined, customElementName: undefined, typicalLifeYears: item.lifespan, lifeReference: item.lifespanLabel, constructionType: "", defects: [] });
  }
  async function addPhotos(files: FileList | null, findingIndex?: number) {
    ...
    for (const file of selectedFiles) {
      ...
      await queueMedia({ id, surveyId, surveyElementId: current.id, findingId: findingIndex === undefined ? undefined : current.defects[findingIndex]?.id, filename: file.name, contentType: imageContentType(file), blob: file, status: "queued", createdAt: new Date().toISOString() });
    }
  ...
  function validationMessage(upToStep = 5) {
    const current = useSurveyDraftStore.getState().draft;
    ...
    if (!current.replacementYearProvided && !current.installationYear) return "Enter the installation year or a known replacement year.";
    if (!current.replacementYearProvided && !current.typicalLifeYears) return "Enter the element life expectancy so remaining life can be calculated.";
    ...
    if (upToStep === 2) return "";
    if (!current.planningHorizon) return "Confirm the planning horizon.";
    if (current.planningHorizon !== lifecycle.suggestedHorizon && !current.planningOverrideReason.trim()) return "Explain why the selected planning horizon differs from the lifecycle calculation.";
    return "";
  }
  function goToStep(target: number) {
    if (target <= step) { setError(""); setStep(target); return; }
  ...
  async function submit(status: "partial" | "completed") {
    ...
    if (savedOnline) {
      const mediaResult = await syncQueuedMedia(payload.id);
      if (mediaResult.failed) {
        setDraft(payload);
        setSaving(false);
        setPhotoError("The element was saved, but one or more photographs could not upload. They remain queued; check the file type or connection and save again.");
        return;
      }
    }
    reset();
  ...
  return <div className="grid gap-5">
    <Card className="overflow-hidden"><div className="overflow-x-auto scrollbar-thin"><ol className="flex min-w-[720px]" aria-label="Survey stages">{steps.map((label, index) => <li key={label} className="flex min-w-[120px] flex-1"><button type="button" onClick={() => goToStep(index)} aria-current={index === step ? "step" : undefined} className={`relative flex w-full items-center gap-3 border-r border-[var(--line)] px-4 py-4 text-left ${index === step ? "bg-blue-50 text-[var(--brand)] dark:bg-blue-950/40" : index < step ? "text-[var(--leaf)] hover:bg-[var(--surface-muted)]" : "text-[var(--ink-muted)] hover:bg-[var(--surface-muted)]"}`}><span className={`grid size-7 shrink-0 place-items-center rounded-lg text-xs font-bold ${index === step ? "bg-[var(--brand)] text-white" : index < step ? "bg-[var(--leaf)] text-white" : "bg-[var(--surface-muted)]"}`}>{index < step ? <Check size={15} weight="bold" /> : index + 1}</span><span className="text-xs font-semibold">{label}</span></button></li>)}</ol></div></Card>
    <Card className="p-5 sm:p-7">{error ? <div className="mb-5 rounded-xl bg-red-50 p-4 text-sm font-medium text-red-800 dark:bg-red-950/60 dark:text-red-200" role="alert">{error}</div> : null}<AnimatePresence mode="wait" initial={false}><motion.div key={step} initial={{ opacity: 0, x: 14 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -14 }} transition={{ duration: 0.2 }}>
      {step === 0 ? <div className="grid gap-6"><SectionIntro title="Choose the element" description="The flat remains selected while you move through each component." /><div className="grid gap-5 sm:grid-cols-2"><Field label="Component area" htmlFor="category" required><Select id="category" value={draft.customComponentName ? "__other__" : draft.category} onChange={(event) => { if (event.target.value === "__other__") patchDraft({ category: "", categoryId: undefined, customComponentName: "", element: "", elementId: undefined, customElementName: "" }); else changeCategory(event.target.value); }}><option value="">Select component</option>{categories.map((category) => <option key={category}>{category}</option>)}<option value="__other__">Other not listed</option></Select>{draft.customComponentName !== undefined ? <Input className="mt-2" value={draft.customComponentName} onChange={(event) => patchDraft({ category: event.target.value, customComponentName: event.target.value })} placeholder="Enter component name" required /> : null}</Field><Field label="Element" htmlFor="element" required><Select id="element" value={draft.customElementName ? "__other__" : draft.element} onChange={(event) => { if (event.target.value === "__other__") patchDraft({ element: "", elementId: undefined, customElementName: "" }); else changeElement(event.target.value); }}><option value="">Select element</option>{catalog.filter((item) => item.category === draft.category).map((item) => <option key={item.name}>{item.name}</option>)}<option value="__other__">Other not listed</option></Select>{draft.customElementName !== undefined ? <Input className="mt-2" value={draft.customElementName} onChange={(event) => patchDraft({ element: event.target.value, customElementName: event.target.value })} placeholder="Enter element name" required /> : null}</Field></div></div> : null}
      {step === 1 ? <div className="grid gap-6"><SectionIntro title="Record construction" description="Choose a controlled type or enter another construction where needed." /><Field label="Construction type" htmlFor="construction" required><Select id="construction" value={constructionOptions.includes(draft.constructionType) ? draft.constructionType : draft.constructionType ? "__other__" : ""} onChange={(event) => patchDraft({ constructionType: event.target.value === "__other__" ? "" : event.target.value })}><option value="">Select construction</option>{constructionOptions.map((type) => <option key={type}>{type}</option>)}<option value="__other__">Other not listed</option></Select>{(!constructionOptions.includes(draft.constructionType) || draft.constructionType === "") ? <Input className="mt-2" value={constructionOptions.includes(draft.constructionType) ? "" : draft.constructionType} onChange={(event) => patchDraft({ constructionType: event.target.value })} placeholder="Enter construction type" required /> : null}</Field><div className="grid gap-5 sm:grid-cols-3"><Field label="Installation year" htmlFor="installationYear" helper="Used with the life expectancy when no replacement year is known."><Input id="installationYear" type="number" min="1600" max={new Date().getFullYear()} value={draft.installationYear ?? ""} onChange={(event) => patchDraft({ installationYear: event.target.value ? Number(event.target.value) : undefined })} /></Field><Field label="Replacement year" htmlFor="replacementYear" helper="When provided, this takes priority in the remaining-life calculation."><Input id="replacementYear" type="number" min="1600" max="2600" value={draft.replacementYearProvided ? draft.replacementYear ?? "" : ""} onChange={(event) => patchDraft({ replacementYear: event.target.value ? Number(event.target.value) : undefined, replacementYearProvided: Boolean(event.target.value) })} /></Field><Field label="Life expectancy" htmlFor="typicalLifeYears" helper={`Reference data: ${draft.lifeReference}`}><Input id="typicalLifeYears" type="number" min="1" value={draft.typicalLifeYears ?? ""} onChange={(event) => patchDraft({ typicalLifeYears: event.target.value ? Number(event.target.value) : undefined })} /></Field></div><Field label="Construction notes" htmlFor="constructionNotes"><Textarea id="constructionNotes" value={draft.constructionNotes} onChange={(event) => patchDraft({ constructionNotes: event.target.value })} placeholder="Material, specification, location or observed alterations." /></Field></div> : null}
      {step === 2 ? <div className="grid gap-6"><div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between"><SectionIntro title="Record defects" description="Each assessment, including no defect observed, records its own condition and priority." /><Button variant="secondary" onClick={addFinding}><Plus size={18} />Add finding</Button></div>{draft.defects.length ? <div className="grid gap-5">{draft.defects.map((finding, index) => <div key={finding.id} className="rounded-2xl border border-[var(--line)] bg-[var(--surface-muted)] p-4 sm:p-5"><div className="mb-5 flex items-center justify-between"><div><Badge tone="blue">Finding {index + 1}</Badge><p className="mt-2 text-xs text-[var(--ink-muted)]">Assessment for {draft.element}</p></div><Button variant="ghost" size="sm" onClick={() => removeFinding(index)} aria-label={`Remove finding ${index + 1}`}><Trash size={18} /></Button></div><div className="grid gap-5 sm:grid-cols-2"><Field label="Defect or deficiency" htmlFor={`defect-${index}`}><Select id={`defect-${index}`} value={finding.defectType === "No defect observed" ? "__none__" : defectOptions.includes(finding.defectType) ? finding.defectType : finding.defectType ? "__other__" : ""} onChange={(event) => patchFinding(index, { defectType: event.target.value === "__none__" ? "No defect observed" : event.target.value === "__other__" ? "" : event.target.value })}><option value="">Select defect</option><option value="__none__">No defect observed</option>{defectOptions.filter((type) => type !== "Other defect" && type !== "No defect observed").map((type) => <option key={type}>{type}</option>)}<option value="__other__">Other not listed</option></Select>{(!defectOptions.includes(finding.defectType) && finding.defectType !== "No defect observed") || finding.defectType === "" ? <Input className="mt-2" value={defectOptions.includes(finding.defectType) || finding.defectType === "Other defect" ? "" : finding.defectType} onChange={(event) => patchFinding(index, { defectType: event.target.value })} placeholder="Optional defect description" /> : null}</Field><Field label="Cause" htmlFor={`cause-${index}`}><Input id={`cause-${index}`} value={finding.cause} onChange={(event) => patchFinding(index, { cause: event.target.value })} placeholder="Known or suspected cause" /></Field><Field label="Condition rating" htmlFor={`condition-${index}`} required><Select id={`condition-${index}`} value={finding.condition} onChange={(event) => patchFinding(index, { condition: event.target.value as SurveyElementDraft["defects"][number]["condition"] })}><option value="">Select condition</option>{conditionOptions.map((item) => <option key={item.value} value={item.value}>{item.label}</option>)}</Select></Field><Field label="Priority rating" htmlFor={`priority-${index}`} required><Select id={`priority-${index}`} value={finding.priority} onChange={(event) => patchFinding(index, { priority: event.target.value as SurveyElementDraft["defects"][number]["priority"] })}><option value="">Select priority</option>{priorityOptions.map((item) => <option key={item.value} value={item.value}>{item.label}</option>)}</Select></Field><div className="sm:col-span-2"><Field label="Finding notes" htmlFor={`notes-${index}`}><Textarea id={`notes-${index}`} value={finding.notes} onChange={(event) => patchFinding(index, { notes: event.target.value })} /></Field></div><div className="sm:col-span-2"><PhotoGallery ids={finding.photoIds} previews={photoPreviews} addLabel="Add photographs" onAdd={(files) => addPhotos(files, index)} onReplace={(id, file) => replacePhoto(id, file, index)} onRemove={(id) => removePhoto(id, index)} error={photoError} /></div></div></div>)}</div> : <div className="rounded-2xl border border-dashed border-[var(--line)] p-8 text-center"><Warning size={28} className="mx-auto text-[var(--orange)]" /><p className="mt-3 font-semibold">No findings recorded</p><p className="mt-1 text-sm text-[var(--ink-muted)]">Add a finding, including No defect observed where appropriate.</p></div>}</div> : null}
      {step === 3 ? <div className="grid gap-6"><SectionIntro title="Confirm lifecycle" description="A supplied replacement year takes priority; otherwise remaining life is derived from installation year and the reference life expectancy." /><div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4"><LifecycleTile label="Component age" value={lifecycle.age === null ? "Not available" : `${lifecycle.age} years`} /><LifecycleTile label="Typical life" value={draft.typicalLifeYears ? `${draft.typicalLifeYears} years` : "Not available"} /><LifecycleTile label="Remaining life" value={lifecycle.remainingLife === null ? "Not available" : `${lifecycle.remainingLife} years`} warning={(lifecycle.remainingLife ?? 1) < 0} /><LifecycleTile label="Replacement due" value={lifecycle.replacementYear ?? "Not available"} /></div><Field label="Planning horizon" htmlFor="planningHorizon" required helper={lifecycle.suggestedHorizon ? `Suggested from remaining life: ${lifecycle.suggestedHorizon}` : "Enter an installation year and life expectancy, or a known replacement year, to calculate a suggestion."}><Select id="planningHorizon" value={draft.planningHorizon} onChange={(event) => patchDraft({ planningHorizon: event.target.value as SurveyElementDraft["planningHorizon"], planningOverrideReason: event.target.value === lifecycle.suggestedHorizon ? "" : draft.planningOverrideReason })}><option value="">Select planning horizon</option><option>Overdue</option><option>1-10 years</option><option>11-20 years</option><option>21-30 years</option><option>Beyond 30 years</option></Select></Field>{draft.planningHorizon && lifecycle.suggestedHorizon && draft.planningHorizon !== lifecycle.suggestedHorizon ? <Field label="Override reason" htmlFor="overrideReason" required><Textarea id="overrideReason" value={draft.planningOverrideReason} onChange={(event) => patchDraft({ planningOverrideReason: event.target.value })} placeholder="Explain why the surveyor-selected horizon differs from the calculated suggestion." /></Field> : null}</div> : null}
