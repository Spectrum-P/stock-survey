...
export async function POST(request: Request) {
  ...
  const { data, error } = await context.supabase!.from("properties").insert({ organization_id: context.organizationId, name: input.name, building_name: input.buildingName || input.name, property_type: input.propertyType || null, address_line_1: input.addressLine1, address_line_2: input.addressLine2 || null, town: input.town, postcode: input.postcode.toUpperCase(), construction_year: input.constructionYear ?? null, reference: input.reference || null }).select("id").single();
