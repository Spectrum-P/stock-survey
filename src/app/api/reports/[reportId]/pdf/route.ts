...
function PdfDocument({ document, title, draft, photoUrls }: { document: ReportDocument; title: string; draft: boolean; photoUrls: Record<string, string> }) {
  ...
  const componentSections = document.componentSections.map((component) => {
    const elements = component.elements.map((element) => React.createElement(View, { key: element.surveyElementId, style: styles.row },
      ...
      React.createElement(View, { style: styles.cell }, React.createElement(Text, null, element.narrative || `${element.element} was assessed. No further narrative was recorded.`), React.createElement(Text, { style: styles.small }, element.defects.length ? `Defects: ${element.defects.join(", ")}` : "No defects were recorded at the time of inspection."), element.recommendedWorks ? React.createElement(Text, { style: styles.small }, `Works: ${element.recommendedWorks}`) : null)));
