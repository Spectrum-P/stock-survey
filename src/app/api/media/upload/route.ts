...
export async function POST(request: Request) {
  ...
  if (!allowedTypes.has(file.type) || file.size > 15 * 1024 * 1024) return jsonError("Upload a JPG, PNG, HEIC or WebP image up to 15 MB", 422);
  ...
  const path = `${context.organizationId}/${survey.property_id}/${survey.unit_id}/${surveyId}/${surveyElementId}/${mediaId}.${extension}`;
  const { error: uploadError } = await context.supabase!.storage.from("survey-media").upload(path, file, { upsert: true, contentType: file.type });
  if (uploadError) return jsonError(uploadError.message, 500);
  const { error } = await context.supabase!.from("media").insert({ id: mediaId, organization_id: context.organizationId, survey_id: surveyId, survey_element_id: surveyElementId, finding_id: findingId, storage_path: path, filename: file.name, content_type: file.type, size_bytes: file.size, uploaded_by: context.user.id });
