...
export async function PUT(request: Request) {
  ...
  if (value.defects.length) {
    const { error: findingError } = await context.supabase!.from("defect_findings").upsert(value.defects.map((finding) => ({ id: finding.id, organization_id: context.organizationId, survey_element_id: value.id, defect_type_label: finding.defectType.trim() || null, cause: finding.cause || null, condition: finding.condition, priority: finding.priority, notes: finding.notes || null })), { onConflict: "id" });
    if (findingError) return jsonError(findingError.message, 500);
  }
  const findingIds = value.defects.map((finding) => finding.id);
  let removedFindings = context.supabase!.from("defect_findings").delete().eq("survey_element_id", value.id).eq("organization_id", context.organizationId);
  if (findingIds.length) removedFindings = removedFindings.not("id", "in", `(${findingIds.join(",")})`);
  const { error: removedFindingError } = await removedFindings;
  if (removedFindingError) return jsonError(removedFindingError.message, 500);
